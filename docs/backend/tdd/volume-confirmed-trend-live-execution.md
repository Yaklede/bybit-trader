...
- 거래소 포지션의 방향 또는 수량이 checkpoint의 기존 소유권 증거와 정확히 일치하지 않으면 HALT,
  승인 철회, 안전 종료, pending 복구 어느 경로에서도 관측값으로 덮어쓰지 않는다. 불일치는 별도 중단
  사유로 기록하고 기존 증거를 보존하므로 다음 주기가 수동·외부 포지션을 봇 소유로 승격할 수 없다.
...
| 루프 실패 알림 callback 장애 | fault injection | 오류 기록 후 다음 대사 주기 계속 실행 |
| Shadow 루프 실패 알림 callback 장애 | fault injection | 오류 기록 후 다음 관측 주기 계속 실행 |
| H4 중요 알림 sink 미전달 | unit/integration | `delivered=false`를 전송 완료로 기록하지 않고 다음 주기 재시도 |
| ticker 30초 초과 지연 또는 5초 초과 미래 시각 | integration | 신규 평가·주문 0개, 안전 중단 |
| 프로세스 shutdown 중 알림 장애 | fault injection | 거래·대사 job 완료 후 알림 시도, 자원 정리 계속 |
...
- 동결 이벤트는 event ID, session ID, protocol fingerprint, symbol, type, event/observation time을 보존하며 런타임이 중복·다른 세션·다른 전략·비인과 시간 순서뿐 아니라 동결 상태의 종료 거래·방향 전환 누적값과 이벤트 수 불일치도 독립적으로 거부한다.
...
- 실거래 신호 loop와 management-only loop는 실패 알림 callback의 비취소 예외를 격리한다. Discord 또는 알림 기록 실패가 발생해도 오류를 로그로 남기고 다음 대사 주기를 계속하며 coroutine 취소만 전파한다.
- Shadow 관측 loop도 동일한 실패 callback 격리 계약을 사용한다. 시장 데이터 오류 알림까지 실패해도 다음 관측 주기를 계속해 전진 검증 세션이 조용히 중단되지 않으며 coroutine 취소만 전파한다.
- Shadow 오류, 승인 상태, Live 주문·중단 알림의 중복 억제 지문은 alert sink가 `delivered=true`를 반환한 뒤에만 기록한다. 전송 거절이나 일시 장애는 같은 중요 알림을 완료 처리하지 않으며 다음 평가 주기에 다시 시도한다.
- Live 주문 가격 ticker는 조회 완료 시각 기준 30초 이내이고 미래 시계 오차가 5초 이내여야 한다. 이 범위를 벗어나면 신호 평가와 신규 주문 전에 `TREND_TICKER_STALE` 또는 `TREND_TICKER_FROM_FUTURE`로 안전 중단한다.
- 프로세스 shutdown hook은 거래·대사·Shadow·readiness job을 먼저 취소하고 완료를 기다린 뒤 보조 수집 loop와 종료 알림을 처리한다. 보조 정지 또는 알림 기록 실패는 다음 shutdown 단계를 막지 않으며 scope·archive·HTTP client 정리는 `finally`에서 수행한다.
...
- 루프 시작 뒤 승인이 상실되면 신규 노출은 만들지 않지만, 이미 기록된 pending 주문을 먼저 복구하고
  ...
  개인 API를 조회하지 않는다. 승인 상실 시 거래소가 이미 flat이면 `OPEN` 또는 `EXIT_NOT_FILLED`에 남은
  과거 포지션·주문 식별자를 정리한다. 미해결 주문 때문에 `HALTED`된 상태는 주문 ID와 원래 사유를
  유지하되 거래소에서 사라진 포지션 관측값만 제거한다.
