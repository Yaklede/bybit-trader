...
| R9 | Private order/execution streams now journal `execId` fills and classify IOC terminal state, but the dynamic trailing/time-exit position policy is not yet restart-persistent in live execution. | A restart while a position is open can lose strategy-management progress even though exchange TP/SL protection remains. | Persist the shared causal position-policy state and restore it from actual fills before promoting a replacement strategy. |
