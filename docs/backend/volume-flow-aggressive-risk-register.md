...
| R2 | Aggressive compounding allows 50-77% peak-to-trough drawdowns. | Live account may become unusable before long-run recovery, especially with futures leverage and margin rules. | Partially resolved at the execution boundary: a persistent fail-closed circuit breaker blocks new entries after the configured UTC daily loss, account drawdown, or consecutive-loss threshold while continuing to manage existing positions. The default limits are 3%, 20%, and three losses. Residual risk: deposits and withdrawals can distort raw-equity drawdown until transaction-ledger reconciliation and unitized NAV are complete. |
