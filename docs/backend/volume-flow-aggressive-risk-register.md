...
| R2 | Aggressive compounding allows 50-77% peak-to-trough drawdowns. | Live account may become unusable before long-run recovery, especially with futures leverage and margin rules. | Resolved at the execution boundary: a persistent fail-closed circuit breaker blocks new entries after the configured UTC daily loss, unitized-NAV drawdown, or consecutive-loss threshold while continuing to manage existing positions. The default limits are 3%, 20%, and three losses. Bybit transaction-log row IDs prevent duplicated cash-flow processing, and deposits/withdrawals change strategy units rather than NAV. Raw equity remains an audit metric. |
...
| R8 | `pause-all` and `emergency-stop` previously changed only bot mode. | A private order could remain live after an operator action. | Resolved in the execution safety coordinator: safe stop cancels active entry orders and closes only unprotected positions; emergency stop cancels entries and submits reduce-only exits for all positions. Both are exchange-readback verified and reconciliation retries pending work. Residual risk is a prolonged exchange outage, which remains `PENDING` or `FAILED` and requires operator review. |
| R9 | Dynamic trailing/time-exit state was not restart-persistent in live execution. | A restart could reset strategy-management progress. | Resolved for non-partial positions: actual-fill causal state and the last processed closed candle are persisted in SQLite, restored after restart, and never advanced across a candle gap. Partial fill and partial-exit execution remain unsupported. |
