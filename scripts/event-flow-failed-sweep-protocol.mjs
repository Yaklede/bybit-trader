...
const EXPECTED_PARENT_RESULT_SHA256 = "7b84b1e4735f2625120836945c623bc9dde8ebfd1ef9df9ec945c79743f96dab";
const EXPECTED_HYPOTHESIS_SHA256 = "0a62f32d55fa39a965eea95a5d5894943dc0faf7f7b9bbf284feee81d3a06756";
const SELECTION_IDS = [
...
export async function loadFailedSweepProtocol(path, primaryProtocolPath, extensionProtocolPath) {
  ...
  return {
    protocol: validateFailedSweepProtocol(
...
export function validateFailedSweepProtocol(protocol, primaryProtocol, extensionProtocol) {
  if (protocol?.protocolId !== "bybit-event-flow-failed-sweep-reversal-v1" ||
      protocol.status !== "PREDECLARED_DEVELOPMENT_HYPOTHESIS_BEFORE_REPLAY") {
    throw new Error("Failed-sweep protocol must be frozen before replay.");
  }
  if (protocol.parentResult?.resultReceiptSha256 !== EXPECTED_PARENT_RESULT_SHA256 ||
      protocol.parentResult?.requiredStatus !== "REJECTED_ACCEPTED_CONTINUATION") {
    throw new Error("Failed-sweep protocol must bind the rejected continuation result.");
  }
  const trials = protocol.trialAccounting;
  if (trials?.priorEvidenceContractCandidates !== 157 || trials.stageCandidates !== 16 ||
      trials.cumulativeCandidates !== 173 || trials.maximumCumulativeCandidates !== 192 ||
      trials.remainingCandidatesAfterStage !== 19) {
    throw new Error("Failed-sweep trial ledger must remain 157 + 16 = 173 of 192.");
  }
  if (sha256(JSON.stringify(protocol.hypothesis)) !== EXPECTED_HYPOTHESIS_SHA256 ||
    ...
    throw new Error("Failed-sweep hypothesis changed after declaration.");
  }
  ...
  if (candidates.length !== 16 || new Set(candidates.map((candidate) => candidate.id)).size !== 16) {
    throw new Error("Failed-sweep grid must contain exactly 16 unique candidates.");
  }
  ...
  if (canonicalJson(protocol.executionContract) !== canonicalJson(primaryProtocol.executionContract) ||
    ...
    throw new Error("Failed-sweep execution and statistics contracts must remain inherited.");
  }
  ...
  if (outcome?.retuneFrom2024Validation !== false ||
    ...
    throw new Error("Failed-sweep development cannot unlock later evidence or execution.");
  }
  return protocol;
}

export function expandFailedSweepCandidates(protocol) {
...
function validateSources(protocol, primaryProtocol, extensionProtocol) {
  ...
  if (protocol.sourceEvidence?.length !== 2 ||
    ...
    throw new Error("Failed-sweep source evidence binding changed.");
  }
  if (primary.blockIds.join(",") !== primaryProtocol.stages.development.primaryBlocks.map((block) => block.id).join(",") ||
    ...
    throw new Error("Failed-sweep source block inventory changed.");
  }
}
...
function validateSchedule(protocol, primaryProtocol, extensionProtocol) {
  ...
  if (schedule?.candidateSelection?.blockIds?.join(",") !== SELECTION_IDS.join(",") ||
    ...
    throw new Error("Failed-sweep selection schedule changed.");
  }
  for (const [index, era] of schedule.validationEras.entries()) {
    if (era.blockIds.join(",") !== VALIDATION_IDS[index].join(",") || era.evaluationDays !== 30) {
      throw new Error("Failed-sweep validation schedule changed.");
    }
  }
  ...
  if (allIds.length !== 28 || new Set(allIds).size !== 28 || allIds.some((id) => !blocks.has(id))) {
    throw new Error("Failed-sweep schedule must contain every development block exactly once.");
  }
  ...
  if (selectionEnd >= validationStart || selectionEnd >= Date.parse("2024-01-01T00:00:00Z")) {
    throw new Error("Failed-sweep selection must end before 2024 validation.");
  }
}

function validateGates(protocol) {
  const audit = protocol.setupFrequencyAudit;
  if (audit?.usedReturnsOrExitOutcomes !== false || audit.selection2023RawSetups !== 484 ||
      audit.validation2024RawSetups !== 154 || audit.auditPerformedBeforeProtocolFreeze !== true) {
    throw new Error("Failed-sweep setup-frequency audit changed.");
  }
  ...
  if (eligibility?.minimumTrades !== 15 || eligibility.minimumProfitFactor !== 1 ||
    ...
    throw new Error("Failed-sweep selection eligibility changed.");
  }
  ...
  if (gate?.minimumTrades !== 12 || gate.minimumLongTrades !== 4 || gate.minimumShortTrades !== 4 ||
    ...
    throw new Error("Failed-sweep validation gate changed.");
  }
}
