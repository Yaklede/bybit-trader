...
import { loadFailedSweepProtocol } from "./event-flow-failed-sweep-protocol.mjs";
...
const DEFAULT_PROTOCOL = "config/bybit-event-flow-failed-sweep-reversal-v1.json";
...
const PARENT_RESULT = "config/bybit-event-flow-accepted-continuation-result-v1.json";
...
export function parseArgs(argv) {
  ...
  return {
    ...
    output: resolve(values.get("output") ?? resolve(
      repositoryRoot,
      "build/research/bybit-event-flow-failed-sweep-reversal-v1-result.json",
    )),
  };
}

export async function runFailedSweep(options, dependencies = {}) {
  ...
  const loaded = await loadFailedSweepProtocol(
    options.protocol,
    primaryProtocolPath,
  ...
  if (sha256(parentResultBytes) !== EXPECTED_PARENT_RESULT_SHA256 ||
      JSON.parse(parentResultBytes).status !== "REJECTED_ACCEPTED_CONTINUATION") {
    throw new Error("Failed-sweep replay requires the committed rejected parent result.");
  }
  ...
  if (receiptHashes.some((hash, index) => hash !== loaded.protocol.sourceEvidence[index].acquisitionReceiptSha256)) {
    throw new Error("Failed-sweep replay source acquisition receipt changed.");
  }
  ...
  for (const source of loaded.protocol.sourceEvidence) {
    ...
    if (databaseSha256 !== source.databaseSha256) {
      throw new Error(`Failed-sweep source database changed: ${source.id}.`);
    }
    ...
    try {
      ...
      for (const id of source.blockIds) {
        ...
        if (definition == null) throw new Error(`Failed-sweep block definition ${id} is missing.`);
  ...
  const evaluation = evaluateFailedSweep(
    replay,
    blocks,
    loaded.protocol,
    primaryProtocol,
...
async function implementationFingerprint(repositoryRoot) {
  const paths = [
    ...
    "scripts/event-flow-failed-sweep-protocol.mjs",
    "scripts/lib/event-flow-failed-sweep-research.mjs",
    "scripts/event-flow-failed-sweep-replay.mjs",
  ];
...
if (invokedPath === import.meta.url) {
  ...
  const result = await runFailedSweep(options);
