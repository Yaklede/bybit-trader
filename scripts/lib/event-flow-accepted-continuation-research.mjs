...
export function runAcceptedContinuationReplay({ blocks, protocol, primaryProtocol, extensionProtocol }) {
  validateAcceptedContinuationProtocol(protocol, primaryProtocol, extensionProtocol);
...
export function evaluateAcceptedContinuation(replay, blocks, protocol, primaryProtocol, extensionProtocol) {
  validateAcceptedContinuationProtocol(protocol, primaryProtocol, extensionProtocol);
  if (replay.candidateCount !== protocol.trialAccounting.stageCandidates || replay.candidates.length !== 16) {
    throw new Error("Accepted continuation evaluation requires all 16 frozen candidates.");
  }
  ...
  const validationTrades = (selectedReplay?.trades ?? [])
    .filter((trade) => validationIds.has(trade.blockId));
  ...
  return {
    schemaVersion: 1,
    status: passed ? "CANDIDATE_FREEZE_REQUIRED" : "REJECTED_ACCEPTED_CONTINUATION",
...
function resolveBlocks(blockById, ids) {
  return ids.map((id) => {
    ...
    if (block == null) throw new Error(`Accepted continuation source block ${id} is missing.`);
    return block;
  });
}
