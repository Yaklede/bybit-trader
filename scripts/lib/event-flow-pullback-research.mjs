...
export function runPullbackReplay({ blocks, protocol, primaryProtocol, extensionProtocol }) {
  validatePullbackProtocol(protocol, primaryProtocol, extensionProtocol);
...
export function evaluatePullback(replay, blocks, protocol, primaryProtocol, extensionProtocol) {
  validatePullbackProtocol(protocol, primaryProtocol, extensionProtocol);
  if (replay.candidateCount !== 18 || replay.candidates.length !== 18) {
    throw new Error("Pullback evaluation requires all 18 frozen candidates.");
  }
  ...
  return {
    schemaVersion: 1,
    status: passed ? "CANDIDATE_FREEZE_REQUIRED" : "REJECTED_PULLBACK_REACCELERATION",
...
function resolveBlocks(blockById, ids) {
  return ids.map((id) => {
    ...
    if (block == null) throw new Error(`Pullback source block ${id} is missing.`);
    return block;
  });
}
