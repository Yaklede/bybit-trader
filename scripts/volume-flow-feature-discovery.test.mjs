...
test("adaptive momentum profile is capped at its predeclared 24 candidates", async () => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "bybit-adaptive-momentum-boundary-"));
  try {
    const databasePath = path.join(directory, "candles.sqlite");
    const windowsPath = path.join(directory, "windows.json");
    const outDirectory = path.join(directory, "out");
    ...
    execFileSync(
      ...
      [
        ...
        outDirectory,
        "--profile",
        "multi-horizon-momentum-adaptive",
        "--maxCandidates",
        "100",
        "--quiet",
        "true",
    ...
    assert.equal(new Set(ranked.map((result) => result.id)).size, 24);
    assert.ok(ranked.every((result) => result.family === "MULTI_HORIZON_MOMENTUM_ADAPTIVE"));
  } finally {
    await fs.rm(directory, { recursive: true, force: true });
  }
});

test("volume-confirmed momentum profile is capped at its predeclared 24 candidates", async () => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "bybit-volume-confirmed-boundary-"));
  try {
    const databasePath = path.join(directory, "candles.sqlite");
    const windowsPath = path.join(directory, "windows.json");
    const outDirectory = path.join(directory, "out");
    ...
    execFileSync(
      ...
      [
        ...
        outDirectory,
        "--profile",
        "multi-horizon-volume-confirmed",
        "--maxCandidates",
        "100",
        "--quiet",
        "true",
    ...
    assert.equal(ranked.length, 24);
    assert.equal(new Set(ranked.map((result) => result.id)).size, 24);
    assert.ok(ranked.every((result) => result.family === "MULTI_HORIZON_VOLUME_CONFIRMED"));
  } finally {
    await fs.rm(directory, { recursive: true, force: true });
  }
});
...
test("M15 regime profile is capped at its predeclared 24 candidates", async () => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "bybit-m15-regime-boundary-"));
  try {
    const databasePath = path.join(directory, "candles.sqlite");
    const windowsPath = path.join(directory, "windows.json");
    const outDirectory = path.join(directory, "out");
    ...
    assert.equal(ranked.length, 24);
    assert.equal(new Set(ranked.map((result) => result.id)).size, 24);
    assert.ok(ranked.every((result) => result.family === "MULTI_HORIZON_M15_REGIME"));
  } finally {
    await fs.rm(directory, { recursive: true, force: true });
  }
});
