...
private fun AggressiveSetup.toDecision(config: VolumeFlowAggressiveBacktestConfig): StrategyDecision {
    ...
    val reasonCodes =
        listOf(
            ...
            "ENTRY_MODE_${config.entryMode.name}",
            "SIDE_${side.name}",
...
private fun findSetup(
    ...
    for (breakoutIndex in index + 1..latestBreakout) {
        ...
        if (
            ...
            val confirmation =
                confirmationIndex(candles, Side.BUY, breakoutIndex, breakout, cluster.high, atr, config) ?: continue
            return buildSetup(
                candles,
        ...
        if (
            ...
            val confirmation =
                confirmationIndex(candles, Side.SELL, breakoutIndex, breakout, cluster.low, atr, config) ?: continue
            return buildSetup(
                candles,
                confirmation.tradeSide,
                confirmation.signalIndex,
