...
- Latest local 1-year BTCUSDT volume-flow tuning snapshot: the current
  ...
  adds `m1_trend_up_breakout_scalp + m1_chop_volume_rejection_scalp` to the
  prior six-leg set. The replay returned `51.80084%` with `2.43591%` max
  drawdown over 52 trades. The previous asymmetric eight-leg candidate returned
  `49.20519%` with `2.43591%` max drawdown over 57 trades. The current candidate
  is still below the daily `0.5%` to `2%` objective, at roughly `0.142%` simple
  average return per
